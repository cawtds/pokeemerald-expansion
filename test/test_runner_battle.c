...
{
    u32 i;
    s8 *scores = AI_THINKING_STRUCT->score;
...
static inline u32 MakeRngValue(const u16 seed)
{
    int i;
    u32 result = seed;
    for (i = 0; i < 16; i++)
    {
        result = ISO_RANDOMIZE1(result);
    }
    return result;
}
...
void Ability_(u32 sourceLine, u32 ability)
{
    ...
    INVALID_IF(ability >= ABILITY_COUNT, "Illegal ability id: %d", ability);
...
s32 MoveGetTarget(s32 battlerId, u32 moveId, struct MoveContext *ctx, u32 sourceLine)
{
    ...
    {
        const struct MoveInfo *move = &gMoveInfo[moveId];

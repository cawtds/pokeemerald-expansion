    ...
    if (itemId != LIST_CANCEL)
    {
        if (sMartInfo.martType == MART_TYPE_NORMAL)
        {
            ConvertIntToDecimalStringN(
                gStringVar1,
                Item_GetPrice(itemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT),
...
static void Task_BuyMenu(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        default:
            ...
            if (sMartInfo.martType == MART_TYPE_NORMAL)
                sShopData->totalCost = (Item_GetPrice(itemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT));
            else
            ...
            else
            {
                if (sMartInfo.martType == MART_TYPE_NORMAL)
                {
                    ...
                    if (Item_GetPocket(itemId) == POCKET_TM_HM)
                    {
...
static void Task_BuyHowManyDialogueHandleInput(u8 taskId)
{
    ...
    {
        sShopData->totalCost = (Item_GetPrice(tItemId) >> IsPokeNewsActive(POKENEWS_SLATEPORT)) * tItemCount;
        BuyMenuPrintItemQuantityAndPrice(taskId);
    }
    else
    {

...
static bool8 CanReplaceMove(void)
{
    if (sMonSummaryScreen->firstMoveIndex == MAX_MON_MOVES
        || sMonSummaryScreen->newMove == MOVE_NONE
        || IsHMMove(sMonSummaryScreen->summary.moves[sMonSummaryScreen->firstMoveIndex]) != TRUE)
        return TRUE;
    else
        return FALSE;
}
...
static void SetTypeSpritePosAndPal(u8 typeId, u8 x, u8 y, u8 spriteArrayId)
{
    ...
    if (typeId < NUMBER_OF_MON_TYPES)
        sprite->oam.paletteNum = GetTypePaletteNum(typeId);
    else
...
static u8 LoadMonGfxAndSprite(struct Pokemon *mon, s16 *state)
{
    ...
    case 0:
        if (gMain.inBattle)
        {
            if (ShouldIgnoreDeoxysForm(DEOXYS_SUMMARY_SCREEN, sMonSummaryScreen->curMonIndex))
                HandleLoadSpecialPokePic_DontHandleDeoxys(TRUE, gMonSpritesGfxPtr->sprites.ptr[B_POSITION_OPPONENT_LEFT], summary->species2, summary->pid);
            else
        ...
        else
        {
            ...
            {
                if (sMonSummaryScreen->monList.mons == gPlayerParty || sMonSummaryScreen->mode == SUMMARY_MODE_BOX || sMonSummaryScreen->handleDeoxys == TRUE)
                    HandleLoadSpecialPokePic(TRUE, MonSpritesGfxManager_GetSpritePtr(MON_SPR_GFX_MANAGER_A, B_POSITION_OPPONENT_LEFT), summary->species2, summary->pid);
                else
                    HandleLoadSpecialPokePic_DontHandleDeoxys(TRUE, MonSpritesGfxManager_GetSpritePtr(MON_SPR_GFX_MANAGER_A, B_POSITION_OPPONENT_LEFT), summary->species2, summary->pid);
            }
        }
        (*state)++;
        return 0xFF;
    case 1:
        paletteTag = GetMonSpritePalTagFromOtIdPersonality(summary->species2, summary->OTID, summary->pid);
        LoadCompressedSpritePaletteWithTag(GetMonSpritePalFromSpeciesAndPersonality(summary->species2, summary->OTID, summary->pid), paletteTag);
        SetMultiuseSpriteTemplateToPokemon(paletteTag, B_POSITION_OPPONENT_LEFT);
        (*state)++;
        return 0xFF;
    }
}
...
{
    enum Ball ball = ItemIdToBallId(GetMonData(mon, MON_DATA_POKEBALL));

    LoadBallGfx(ball);
    sMonSummaryScreen->spriteIds[SPRITE_ARR_ID_BALL] = CreateSprite(Ball_GetSpriteTemplate(ball), 16, 136, 0);

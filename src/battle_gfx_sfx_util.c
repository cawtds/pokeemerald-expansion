...
u16 ChooseMoveAndTargetInBattlePalace(u32 battler)
{
    ...
    for (; i < maxGroupNum; i++)
    {
        if (gNatureInfo[GetNatureFromPersonality(gBattleMons[battler].personality)].battlePalacePercents[i] > percent)
            break;
    }
    ...
    if (selectedMoves != 0)
    {
        ...
        BattleAI_SetupAIData(battler, selectedMoves);
    ...
    {
        moveTarget = GetMoveTarget(moveInfo->moves[chosenMoveId]);
    }
...
static u16 GetBattlePalaceTarget(u32 battler)
{
    if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE)
    {
        ...
        switch (gNatureInfo[GetNatureFromPersonality(gBattleMons[battler].personality)].battlePalaceSmokescreen)
        {
...
void BattleLoadOpponentMonSpriteGfx(struct Pokemon *mon, u8 battler)
{
    ...
    HandleLoadSpecialPokePic_DontHandleDeoxys(TRUE, gMonSpritesGfxPtr->sprites.ptr[position], species, currentPersonality);
...
void BattleLoadPlayerMonSpriteGfx(struct Pokemon *mon, u8 battler)
{
    ...
    if (ShouldIgnoreDeoxysForm(DEOXYS_PLAYER_SIDE, battler) == TRUE || gBattleSpritesDataPtr->battlerData[battler].transformSpecies != SPECIES_NONE)
    {
        HandleLoadSpecialPokePic_DontHandleDeoxys(FALSE, gMonSpritesGfxPtr->sprites.ptr[position], species, currentPersonality);
    }
    else
    {
        HandleLoadSpecialPokePic(FALSE, gMonSpritesGfxPtr->sprites.ptr[position], species, currentPersonality);
    }
...
void HandleSpeciesGfxDataChange(u8 battlerAtk, u8 battlerDef, bool8 castform)
{
    ...
    else
    {
        ...
        if (IsContest())
        {
            ...
            HandleLoadSpecialPokePic_DontHandleDeoxys(FALSE, gMonSpritesGfxPtr->sprites.ptr[position], targetSpecies, gContestResources->moveAnim->targetPersonality);
        }
        else
        {
            ...
            if (GetBattlerSide(battlerAtk) == B_SIDE_PLAYER)
            {
                ...
                HandleLoadSpecialPokePic_DontHandleDeoxys(FALSE, gMonSpritesGfxPtr->sprites.ptr[position], targetSpecies, gTransformedPersonalities[battlerAtk]);
            }
            else
            {
                ...
                HandleLoadSpecialPokePic_DontHandleDeoxys(TRUE, gMonSpritesGfxPtr->sprites.ptr[position], targetSpecies, gTransformedPersonalities[battlerAtk]);
            }
        }
        ...
        {
            gSprites[gBattlerSpriteIds[battlerAtk]].anims = gSpeciesInfo[targetSpecies].frontAnimFrames;
            LZDecompressWram(lzPaletteData, gBattleStruct->castformPalette);
...
void SpriteCB_EnemyShadow(struct Sprite *shadowSprite)
{
    ...
    if (gAnimScriptActive || battlerSprite->invisible)
        invisible = TRUE;
             ...
             && gSpeciesInfo[gBattleSpritesDataPtr->battlerData[battler].transformSpecies].enemyMonElevation == 0)
        invisible = TRUE;

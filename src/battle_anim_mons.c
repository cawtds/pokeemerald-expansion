...
u8 GetBattlerYDelta(u8 battler, u16 species)
{
    ...
    else
    {
        ...
        {
            ret = gCastformFormInfo[gBattleMonForms[battler]].y_offset;
        }
        else if (species > NUM_SPECIES)
        {
    ...
    if (GetBattlerSide(battler) == B_SIDE_OPPONENT)
    {
        if (!IsContest())
        {
            if (species == SPECIES_CASTFORM)
                ret = gCastformFormInfo[gBattleMonForms[battler]].elevation;
            else if (species > NUM_SPECIES)
        ...
        if (gBattlerSpriteIds[i] == spriteId)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (species == SPECIES_CASTFORM)
                        return gCastformFormInfo[gBattleMonForms[battler]].elevation;
                    else
...
u8 CreateAdditionalMonSpriteForMoveAnim(u16 species, bool8 isBackpic, u8 id, s16 x, s16 y, u8 subpriority, u32 personality, u32 trainerId, u32 battler, bool32 ignoreDeoxysForm)
{
    ...
    if (!isBackpic)
    {
        ...
        if (ignoreDeoxysForm == TRUE || ShouldIgnoreDeoxysForm(DEOXYS_MOVE_ANIM, battler) == TRUE || gBattleSpritesDataPtr->battlerData[battler].transformSpecies != 0)
            LoadSpecialPokePic_DontHandleDeoxys(gMonSpritesGfxPtr->buffer, species, personality, TRUE);
        else
    ...
    else
    {
        LoadCompressedPalette(GetMonSpritePalFromSpeciesAndPersonality(species, trainerId, personality), OBJ_PLTT_ID(palette), PLTT_SIZE_4BPP);
        if (ignoreDeoxysForm == TRUE || ShouldIgnoreDeoxysForm(DEOXYS_MOVE_ANIM, battler) == TRUE || gBattleSpritesDataPtr->battlerData[battler].transformSpecies != 0)
            LoadSpecialPokePic_DontHandleDeoxys(gMonSpritesGfxPtr->buffer, species, personality, FALSE);
        else
...
s16 GetBattlerSpriteCoordAttr(u8 battler, u8 attr)
{
    ...
    if (IsContest())
    {
        ...
        {
            size = gCastformFormInfo[gBattleMonForms[battler]].size;
            y_offset = gCastformFormInfo[gBattleMonForms[battler]].y_offset;
        }

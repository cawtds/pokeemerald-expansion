...
#define DEFENDER_IS_PROTECTED ((gProtectStructs[gBattlerTarget].protected) && IsMoveAffectedByProtect(gCurrentMove))
...
static void Cmd_attackcanceler(void)
{
    ...
    if (gProtectStructs[gBattlerTarget].bounceMove && IsMoveAffectedByMagicCoat(gCurrentMove))
    {
    ...
    {
        if ((gProtectStructs[gBattlerByTurnOrder[i]].stealMove) && IsMoveAffectedBySnatch(gCurrentMove))
        {
...
static bool8 AccuracyCalcHelper(u16 move)
{
    ...
    if ((WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_RAIN) && GetMoveEffect(move) == EFFECT_THUNDER)
     || (GetMoveEffect(move) == EFFECT_ALWAYS_HIT || GetMoveEffect(move) == EFFECT_VITAL_THROW))
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
...
static void Cmd_accuracycheck(void)
{
    ...
    else
    {
        ...
        if (WEATHER_HAS_EFFECT && gBattleWeather & B_WEATHER_SUN && GetMoveEffect(move) == EFFECT_THUNDER)
            moveAcc = 50;
        ...
        // final calculation
        if (!RandomPercentage(RNG_ACCURACY, calc))
        {
            gMoveResultFlags |= MOVE_RESULT_MISSED;
...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 2 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                + (GetMoveEffect(gCurrentMove) == EFFECT_HIGH_CRITICAL)
                + (GetMoveEffect(gCurrentMove) == EFFECT_SKY_ATTACK)
                + (GetMoveEffect(gCurrentMove) == EFFECT_BLAZE_KICK)
                + (GetMoveEffect(gCurrentMove) == EFFECT_POISON_TAIL)
    ...
    if ((gBattleMons[gBattlerTarget].ability != ABILITY_BATTLE_ARMOR && gBattleMons[gBattlerTarget].ability != ABILITY_SHELL_ARMOR)
     ...
     && RandomChance(RNG_CRITICAL_HIT, 1, sCriticalHitChance[critChance]))
        gCritMultiplier = 2;
    else
        gCritMultiplier = 1;
...
void AI_CalcDmg(u8 attacker, u8 defender)
{
    ...
    if (gStatuses3[attacker] & STATUS3_CHARGED_UP && GetMoveType(gCurrentMove) == TYPE_ELECTRIC)
        gBattleMoveDamage *= 2;
...
static bool32 IsForesightedMoveAgainstType(u32 moveType, u32 defendingType, u32 foresightTarget)
{
    return (moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL)
        && defendingType == TYPE_GHOST
        && gBattleMons[foresightTarget].status2 & STATUS2_FORESIGHT;
}
...
static void Cmd_typecalc(void)
{
    ...
    {
        u8 defType1 = gBattleMons[gBattlerTarget].types[0];
        u8 defType2 = gBattleMons[gBattlerTarget].types[1];
        if (!IsForesightedMoveAgainstType(moveType, defType1, gBattlerTarget))
        {
            ModulateDmgByType(GetTypeEffectiveness(moveType, defType1));
        }
        ...
        if (defType1 != defType2)
        {
            ...
            {
                ModulateDmgByType(GetTypeEffectiveness(moveType, defType2));
            }
        }
    }
...
static void CheckWonderGuardAndLevitate(void)
{
    ...
    if (!IsForesightedMoveAgainstType(moveType, defType1, gBattlerTarget))
    {
        multiplier = GetTypeEffectiveness(moveType, defType1);
        switch(multiplier)
        {
            case TYPE_MUL_NO_EFFECT:
                gMoveResultFlags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender)
{
    ...
    {
        u8 defType1 = gBattleMons[defender].types[0];
        u8 defType2 = gBattleMons[defender].types[1];
        // check type1
        if (!IsForesightedMoveAgainstType(moveType, defType1, defender))
        {
            ModulateDmgByType2(GetTypeEffectiveness(moveType, defType1), move, &flags);
        }
        
        // check type2
        if (defType1 != defType2)
        {
            if (!IsForesightedMoveAgainstType(moveType, defType2, defender))
            {
                ModulateDmgByType2(GetTypeEffectiveness(moveType, defType2), move, &flags);
            }
        }
    }
...
u8 AI_TypeCalc(u16 move, u16 targetSpecies, u8 targetAbility)
{
    ...
    {
        // check type1
        ModulateDmgByType2(GetTypeEffectiveness(moveType, type1), move, &flags);
        
        // check type2
        if (type1 != type2)
        {
            ModulateDmgByType2(GetTypeEffectiveness(moveType, type2), move, &flags);
        }
    }
...
static void Cmd_adjustnormaldamage(void)
{
    ...
    if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_SUBSTITUTE)
     && (GetMoveEffect(gCurrentMove) == EFFECT_FALSE_SWIPE || gProtectStructs[gBattlerTarget].endured || gSpecialStatuses[gBattlerTarget].focusBanded)
...
static void Cmd_attackanimation(void)
{
    ...
    else
    {
        ...
        if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
        {
            ...
            BtlController_EmitMoveAnimation(gBattlerAttacker, B_COMM_TO_CONTROLLER, gCurrentMove, gBattleScripting.animTurn, gBattleMovePower, gBattleMoveDamage, gBattleMons[gBattlerAttacker].friendship, &gDisableStructs[gBattlerAttacker], multihit);
            gBattleScripting.animTurn++;
            gBattleScripting.animTargetsHit++;
            MarkBattlerForControllerExec(gBattlerAttacker);
            gBattlescriptCurrInstr++;
...
static void Cmd_healthbarupdate(void)
{
    ...
    if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
    {
        u32 battler = GetBattlerForBattleScript(gBattlescriptCurrInstr[1]);

        if (gBattleMons[battler].status2 & STATUS2_SUBSTITUTE && gDisableStructs[battler].substituteHP && !(gHitMarker & HITMARKER_IGNORE_SUBSTITUTE))
        {
...
static void Cmd_printselectionstring(void)
{
    BtlController_EmitPrintSelectionString(gBattlerAttacker, B_COMM_TO_CONTROLLER, T2_READ_16(gBattlescriptCurrInstr + 1));
    MarkBattlerForControllerExec(gBattlerAttacker);
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        case STATUS1_SLEEP:
            ...
            {
                for (battler = 0; battler < gBattlersCount && !(gBattleMons[battler].status2 & STATUS2_UPROAR); battler++)
                {}
            }
            else
            {
            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                    ...
                    else
                    {
                        ...
                        BtlController_EmitSetMonData(gBattlerAttacker, B_COMM_TO_CONTROLLER, REQUEST_HELDITEM_BATTLE, 0, sizeof(gLastUsedItem), &gLastUsedItem);
                        MarkBattlerForControllerExec(gBattlerAttacker);
...
static void Cmd_seteffectwithchance(void)
{
    ...
    if (gBattleMons[gBattlerAttacker].ability == ABILITY_SERENE_GRACE)
        percentChance = GetMoveSecondaryEffectChance(gCurrentMove) * 2;
    else
        percentChance = GetMoveSecondaryEffectChance(gCurrentMove);
...
static void Cmd_clearstatusfromeffect(void)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
        gBattleMons[battler].status1 &= (~sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]]);
    else
        gBattleMons[battler].status2 &= (~sStatusFlagsForMoveEffects[gBattleCommunication[MOVE_EFFECT_BYTE]]);
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
        ...
        {
            battlerTarget = gBattlerAttacker;
            battlerAttacker = gBattlerTarget;
            BS_ptr = BattleScript_FaintAttacker;
        }
        else
        {
            battlerTarget = gBattlerTarget;
            battlerAttacker = gBattlerAttacker;
            BS_ptr = BattleScript_FaintTarget;
        }
        if (!(gAbsentBattlerFlags & gBitTable[battlerTarget])
         && gBattleMons[battlerTarget].hp == 0)
        {
            gHitMarker |= HITMARKER_FAINTED(battlerTarget);
            BattleScriptPush(gBattlescriptCurrInstr + 7);
            gBattlescriptCurrInstr = BS_ptr;
            if (GetBattlerSide(battlerTarget) == B_SIDE_PLAYER)
            {
                gHitMarker |= HITMARKER_PLAYER_FAINTED;
                if (gBattleResults.playerFaintCounter < 255)
                    gBattleResults.playerFaintCounter++;
                AdjustFriendshipOnBattleFaint(battlerTarget);
            }
            else
            {
                if (gBattleResults.opponentFaintCounter < 255)
                    gBattleResults.opponentFaintCounter++;
                gBattleResults.lastOpponentSpecies = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battlerTarget]], MON_DATA_SPECIES, NULL);
            }
            if ((gHitMarker & HITMARKER_DESTINYBOND) && gBattleMons[gBattlerAttacker].hp != 0)
            {
                gHitMarker &= ~HITMARKER_DESTINYBOND;
                BattleScriptPush(gBattlescriptCurrInstr);
                gBattleMoveDamage = gBattleMons[battlerAttacker].hp;
...
static void Cmd_getexp(void)
{
    ...
    case 3: // Set stats and give exp
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP) && GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_LEVEL) != MAX_LEVEL)
            {
                ...
                BtlController_EmitExpUpdate(gBattleStruct->expGetterBattlerId, B_COMM_TO_CONTROLLER, gBattleStruct->expGetterMonId, gBattleMoveDamage);
    ...
    case 4: // lvl up if necessary
        if (gBattleControllerExecFlags == 0)
        {
            u32 battler = gBattleStruct->expGetterBattlerId;
            if (gBattleBufferB[battler][0] == CONTROLLER_TWORETURNVALUES && gBattleBufferB[battler][1] == RET_VALUE_LEVELED_UP)
            {
                ...
                PREPARE_MON_NICK_WITH_PREFIX_BUFFER(gBattleTextBuff1, battler, gBattleStruct->expGetterMonId);
...
static void Cmd_healthbar_update(void)
{
    ...
    BtlController_EmitHealthBarUpdate(battler, B_COMM_TO_CONTROLLER, gBattleMoveDamage);
    MarkBattlerForControllerExec(battler);
    gBattlescriptCurrInstr += 2;
}
...
static void Cmd_playanimation(void)
{
    ...
    {
        BtlController_EmitBattleAnimation(battler, B_COMM_TO_CONTROLLER, gBattlescriptCurrInstr[2], *argumentPtr);
        MarkBattlerForControllerExec(battler);
        gBattlescriptCurrInstr += 7;
    }
...
static void Cmd_playanimation_var(void)
{
    ...
    {
        BtlController_EmitBattleAnimation(battler, B_COMM_TO_CONTROLLER, *animationIdPtr, *argumentPtr);
        MarkBattlerForControllerExec(battler);
        gBattlescriptCurrInstr += 10;
    }
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
...
static void Cmd_playstatchangeanimation(void)
{
            ...
            if (statsToCheck & 1)
            {
                if (gBattlescriptCurrInstr[3] & STAT_CHANGE_CANT_PREVENT)
                {
                    if (gBattleMons[battler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        ...
                        && !(gBattleMons[battler].ability == ABILITY_HYPER_CUTTER && currStat == STAT_ATK))
                {
                    if (gBattleMons[battler].statStages[currStat] > MIN_STAT_STAGE)
                    {
                        statAnimId = startingStatAnimId + currStat;
    ...
    else // goes up
    {
        ...
        while (statsToCheck != 0)
        {
            if (statsToCheck & 1 && gBattleMons[battler].statStages[currStat] < MAX_STAT_STAGE)
            {
                statAnimId = startingStatAnimId + currStat;
                changeableStatsCount++;
            }
    ...
    do
    {
        ...
        case MOVEEND_UPDATE_LAST_MOVES:
            ...
            if (!(gAbsentBattlerFlags & gBitTable[gBattlerAttacker])
                ...
                && GetMoveEffect(originallyUsedMove) != EFFECT_BATON_PASS)
            {
                if (gHitMarker & HITMARKER_OBEYS)
                {
        ...
        case MOVEEND_MIRROR_MOVE: // mirror move
            if (!(gAbsentBattlerFlags & gBitTable[gBattlerAttacker])
                && !(gBattleStruct->absentBattlerFlags & gBitTable[gBattlerAttacker])
                && IsMoveAffectedByMirrorMove(originallyUsedMove)
                && gHitMarker & HITMARKER_OBEYS
                && gBattlerAttacker != gBattlerTarget
...
static void Cmd_typecalc2(void)
{
    ...
    else
    {
        ...
        if (!IsForesightedMoveAgainstType(moveType, defType1, gBattlerTarget))
        {
            u8 multiplier = GetTypeEffectiveness(moveType, defType1);
            switch (multiplier)
            {
                case TYPE_MUL_NO_EFFECT:
                    gMoveResultFlags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
        ...
        if (defType1 != defType2)
        {
            if (!IsForesightedMoveAgainstType(moveType, defType2, gBattlerTarget))
            {
                u8 multiplier = GetTypeEffectiveness(moveType, defType2);
                switch (multiplier)
                {
                    case TYPE_MUL_NO_EFFECT:
                        gMoveResultFlags |= MOVE_RESULT_DOESNT_AFFECT_FOE;
...
static void Cmd_getswitchedmondata(void)
{
    ...
    BtlController_EmitGetMonData(battler, B_COMM_TO_CONTROLLER, REQUEST_ALL_BATTLE, gBitTable[gBattlerPartyIndexes[battler]]);
    MarkBattlerForControllerExec(battler);
...
static void Cmd_switchinanim(void)
{
    CMD_ARGS(u8 battler, u8 dontClearSubstituteBit);
    u32 battler;
    ...
    BtlController_EmitSwitchInAnim(battler, B_COMM_TO_CONTROLLER, gBattlerPartyIndexes[battler], cmd->dontClearSubstituteBit);
    MarkBattlerForControllerExec(battler);
...
static void Cmd_jumpifcantswitch(void)
{
    ...
    battler = GetBattlerForBattleScript(cmd->battler & ~SWITCH_IGNORE_ESCAPE_PREVENTION);
...
static void Cmd_openpartyscreen(void)
{
    ...
    {
        if (cmd->battler & PARTY_SCREEN_OPTIONAL)
            hitmarkerFaintBits = PARTY_ACTION_CHOOSE_MON; // Used here as the caseId for the EmitChoose function.
        else
            hitmarkerFaintBits = PARTY_ACTION_SEND_OUT;

        battler = GetBattlerForBattleScript(cmd->battler & ~PARTY_SCREEN_OPTIONAL);
        ...
        else
        {
            ...
            {
                battler = GetBattlerAtPosition(BATTLE_OPPOSITE(GetBattlerPosition(battler)));
...
static void Cmd_switchhandleorder(void)
{
    ...
    case 2:
        ...
        {
            RecordedBattle_SetBattlerAction(battler, gBattleBufferB[battler][1]);
            gBattleStruct->field_93 |= gBitTable[battler];
        }
        // fall through
    case 3:
        ...
        {
            gBattleStruct->battlerPartyOrders[battler][0] &= 0xF;
            gBattleStruct->battlerPartyOrders[battler][0] |= (gBattleBufferB[battler][2] & 0xF0);
            gBattleStruct->battlerPartyOrders[battler][1] = gBattleBufferB[battler][3];

            gBattleStruct->battlerPartyOrders[BATTLE_PARTNER(battler)][0] &= (0xF0);
            gBattleStruct->battlerPartyOrders[BATTLE_PARTNER(battler)][0] |= (gBattleBufferB[battler][2] & 0xF0) >> 4;
            gBattleStruct->battlerPartyOrders[BATTLE_PARTNER(battler)][2] = gBattleBufferB[battler][3];
        }
        else if (gBattleTypeFlags & BATTLE_TYPE_INGAME_PARTNER)
        {
...
static void Cmd_switchineffects(void)
{
    ...
    if (!(gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SPIKES_DAMAGED)
        ...
        && gBattleMons[battler].ability != ABILITY_LEVITATE)
    {
        ...
        spikesDmg = (5 - gSideTimers[GetBattlerSide(battler)].spikesAmount) * 2;
        ...
        else if (cmd->battler == BS_ATTACKER)
    ...
    else
    {
        ...
        if (gBattleMons[battler].ability == ABILITY_TRUANT && !gDisableStructs[battler].truantSwitchInHack)
...
static void Cmd_handlelearnnewmove(void)
{
    CMD_ARGS(u8* learnedMovePtr, u8* nothingToLearnPtr, bool8 isFirstMove);
...
static void Cmd_yesnoboxstoplearningmove(void)
{
    ...
    case 1:
        ...
        if (JOY_NEW(A_BUTTON))
        {
            ...
            if (gBattleCommunication[1] != 0)
                gBattlescriptCurrInstr = cmd->noPtr;
            else
                gBattlescriptCurrInstr = cmd->nextInstr;
...
static void Cmd_hitanimation(void)
{
    ...
    else if (!(gHitMarker & HITMARKER_IGNORE_SUBSTITUTE) || !(gBattleMons[battler].status2 & STATUS2_SUBSTITUTE) || gDisableStructs[battler].substituteHP == 0)
    {
...
static void Cmd_status2animation(void)
{
    ...
    if (gBattleControllerExecFlags == 0)
    {
        ...
        if (!(gStatuses3[battler] & STATUS3_SEMI_INVULNERABLE)
            ...
            && !(gHitMarker & HITMARKER_NO_ANIMATIONS))
        {
            BtlController_EmitStatusAnimation(battler, B_COMM_TO_CONTROLLER, TRUE, gBattleMons[battler].status2 & wantedToAnimate);
            MarkBattlerForControllerExec(battler);
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void Cmd_removeitem(void)
{
    ...
    usedHeldItem = &gBattleStruct->usedHeldItems[battler];
    *usedHeldItem = gBattleMons[battler].item;
    gBattleMons[battler].item = ITEM_NONE;
...
static void Cmd_various(void)
{
    ...
    case VARIOUS_PALACE_FLAVOR_TEXT:
            ...
            && !(gBattleMons[battler].status1 & STATUS1_SLEEP))
        {
            gBattleStruct->palaceFlags |= gBitTable[battler];
            gBattleCommunication[0] = TRUE;
            gBattleCommunication[MULTISTRING_CHOOSER] = Nature_GetBattlePalaceFlavorText(GetNatureFromPersonality(gBattleMons[battler].personality));
        }
        break;
...
static void Cmd_setprotectlike(void)
{
    ...
    {
        if (GetMoveEffect(gCurrentMove) == EFFECT_PROTECT)
        {
...
static void Cmd_trysetrest(void)
{
    ...
    if (gBattleMons[gBattlerTarget].hp == gBattleMons[gBattlerTarget].maxHP)
    {
        gBattlescriptCurrInstr = cmd->failJump;
    }
    else
    {
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((gBattleMons[battler].ability == ABILITY_CLEAR_BODY
                  || gBattleMons[battler].ability == ABILITY_WHITE_SMOKE)
                 && !certain && gCurrentMove != MOVE_CURSE)
        {
            if (flags == STAT_CHANGE_ALLOW_PTR)
        ...
        else if (gBattleMons[battler].ability == ABILITY_KEEN_EYE
        ...
        else if (gBattleMons[battler].ability == ABILITY_HYPER_CUTTER
        ...
        else if (gBattleMons[battler].ability == ABILITY_SHIELD_DUST && flags == 0)
        {
            return STAT_CHANGE_DIDNT_WORK;
        }
...
static void Cmd_statbuffchange(void)
{
    CMD_ARGS(u8 flags, u8 *jumpPtr);

    if (ChangeStatBuffs(gBattleScripting.statChanger & 0xF0, GET_STAT_BUFF_ID(gBattleScripting.statChanger), cmd->flags, cmd->jumpPtr) == STAT_CHANGE_WORKED)
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_tryKO(void)
{
    ...
    else
    {
        ...
        {
            chance = GetMoveAccuracy(gCurrentMove) + (gBattleMons[gBattlerAttacker].level - gBattleMons[gBattlerTarget].level);
            if (RandomPercentage(RNG_ACCURACY, chance) && gBattleMons[gBattlerAttacker].level >= gBattleMons[gBattlerTarget].level)
                chance = TRUE;
            else
                chance = FALSE;
        }
...
static void Cmd_updatestatusicon(void)
{
    ...
    else
    {
        if (!(gAbsentBattlerFlags & gBitTable[gBattlerAttacker]))
        {
            BtlController_EmitStatusIconUpdate(gBattlerAttacker, B_COMM_TO_CONTROLLER, gBattleMons[gBattlerAttacker].status1, gBattleMons[gBattlerAttacker].status2);
            MarkBattlerForControllerExec(gBattlerAttacker);
        }
        if ((gBattleTypeFlags & BATTLE_TYPE_DOUBLE))
        {
            u32 battlerPartner = GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(gBattlerAttacker)));
            if (!(gAbsentBattlerFlags & gBitTable[battlerPartner]))
            {
                BtlController_EmitStatusIconUpdate(battlerPartner, B_COMM_TO_CONTROLLER, gBattleMons[battlerPartner].status1, gBattleMons[battlerPartner].status2);
                MarkBattlerForControllerExec(battlerPartner);
            }
        }
        gBattlescriptCurrInstr = cmd->nextInstr;
    }
...
static void Cmd_transformdataexecution(void)
{
    ...
    else
    {
        ...
        for (i = 0; i < MAX_MON_MOVES; i++)
        {
            if (GetMovePP(gBattleMons[gBattlerAttacker].moves[i]) < 5)
                gBattleMons[gBattlerAttacker].pp[i] = GetMovePP(gBattleMons[gBattlerAttacker].moves[i]);
            else
...
static void Cmd_mimicattackcopy(void)
{
    ...
    else
    {
        ...
        if (i == MAX_MON_MOVES)
        {
            ...
            if (GetMovePP(gLastMoves[gBattlerTarget]) < 5)
                gBattleMons[gBattlerAttacker].pp[gCurrMovePos] = GetMovePP(gLastMoves[gBattlerTarget]);
            else
...
static bool8 IsTwoTurnsMove(u16 move)
{
    if (GetMoveEffect(move) == EFFECT_SKULL_BASH
     || GetMoveEffect(move) == EFFECT_RAZOR_WIND
     || GetMoveEffect(move) == EFFECT_SKY_ATTACK
     || GetMoveEffect(move) == EFFECT_SOLAR_BEAM
     || GetMoveEffect(move) == EFFECT_SEMI_INVULNERABLE
     || GetMoveEffect(move) == EFFECT_BIDE)
        return TRUE;
    else
        return FALSE;
}
...
static void Cmd_trychoosesleeptalkmove(void)
{
    ...
    else // at least one move can be chosen
    {
        ...
        gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
        gBattlerTarget = GetMoveTargetBattler(gCalledMove, NO_TARGET_OVERRIDE);
        gBattlescriptCurrInstr = cmd->ptr;
    }
}
...
static void Cmd_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE
            && !(gAbsentBattlerFlags & gBitTable[battler]))
        {
            if (gBattleMons[battler].ability != ABILITY_SOUNDPROOF)
            {
                gBattleMons[battler].status1 = 0;
...
static void Cmd_jumpifconfusedandstatmaxed(void)
{
    CMD_ARGS(u8 stat, u8 *ptr);

    if (gBattleMons[gBattlerTarget].status2 & STATUS2_CONFUSION
        && gBattleMons[gBattlerTarget].statStages[cmd->stat] == MAX_STAT_STAGE)
        gBattlescriptCurrInstr = cmd->ptr;
    else
        gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_friendshiptodamagecalculation(void)
{
    ...
    if (GetMoveEffect(gCurrentMove) == EFFECT_RETURN)
...
static void Cmd_callenvironmentattack(void)
{
    ...
    gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
    gCurrentMove = BattleTerrain_GetNaturePowerMove(gBattleEnvironment);
    gBattlerTarget = GetMoveTargetBattler(gCurrentMove, NO_TARGET_OVERRIDE);
    BattleScriptPush(GetMoveBattleScript(gCurrentMove));
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_weightdamagecalculation(void)
{
    ...
    {
        if (sWeightToDamageTable[i] > GetSpeciesWeight(gBattleMons[gBattlerTarget].species))
            break;
    }
...
static void Cmd_getsecretpowereffect(void)
{
    ...
    gBattleCommunication[MOVE_EFFECT_BYTE] = BattleTerrain_GetSecretPowerEffect(gBattleEnvironment);
    gBattlescriptCurrInstr = cmd->nextInstr;
}
...
static void Cmd_docastformchangeanimation(void)
{
    ...
    BtlController_EmitBattleAnimation(battler, B_COMM_TO_CONTROLLER, B_ANIM_CASTFORM_CHANGE, gBattleStruct->formToChangeInto);
    MarkBattlerForControllerExec(battler);
...
static void Cmd_tryrecycleitem(void)
{
    ...
    usedHeldItem = &gBattleStruct->usedHeldItems[gBattlerAttacker];
...
static void Cmd_settypetoenvironment(void)
{
    ...
    if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, BattleTerrain_GetCamouflageType(gBattleEnvironment)))
    {
        SET_BATTLER_TYPE(gBattlerAttacker, BattleTerrain_GetCamouflageType(gBattleEnvironment));
        PREPARE_TYPE_BUFFER(gBattleTextBuff1, BattleTerrain_GetCamouflageType(gBattleEnvironment));

...
static void PrintItemDescription(s32 listMenuId)
{
    ...
    {
        desc = Item_GetDescription(gSaveBlock2Ptr->frontier.pyramidBag.itemId[gSaveBlock2Ptr->frontier.lvlMode][listMenuId]);
    }
    else
    {
...
{
    u8 pocketId = Item_GetPocket(gSpecialVar_ItemId);
    ...
    else if (Item_GetFieldFunc(gSpecialVar_ItemId) != NULL)
    {
        CloseMenuActionWindow();
        FillWindowPixelBuffer(WIN_INFO, PIXEL_FILL(0));
...
{
    if (Item_GetBattleFunc(gSpecialVar_ItemId) != NULL)
    {
        CloseMenuActionWindow();

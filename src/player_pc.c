...
static void ItemStorage_PrintDescription(s32 id)
{
    ...
    if (id != LIST_CANCEL)
        description = (u8 *)Item_GetDescription(gSaveBlock1Ptr->pcItems[id].itemId);
    else
...
static const u8 *ItemStorage_GetMessage(u16 itemId)
{
    ...
    case MSG_SWITCH_WHICH_ITEM:
        ...
        break;
    default:
        string = Item_GetDescription(itemId);
        break;
    }
    return string;
}
...
static void ItemStorage_DoItemToss(u8 taskId)
{
    ...
    if (!Item_GetImportance(gSaveBlock1Ptr->pcItems[pos].itemId))
    {
